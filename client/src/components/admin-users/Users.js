...
const Clients = ({
	...
	useEffect(() => {
		...
		setData({
			userData: loading || !clients ? [] : filterClients(clients),
		});
	}, [getPackageUsers, loading, loadUser]);

	const filterClients = (clients) => clients.filter((client) => client.role !== 'admin');

	const onChange = (e) => {
		let filterSearch = clients.filter((val) => {
			let name = `${val.firstName} ${val.lastName}`;
			if (name.toLowerCase().indexOf(e.target.value) > -1 && val.role !== 'admin') {
				return val;
			}
		});
			...
			{loading ? (
				...
				<Fragment>
					...
					<div className="dash-buttons">
						<Link to="/admin-manage-booking" className="btn btn-light">
							<i className="fas fa-calendar-check"></i> Manage Booking
						</Link>
						<Link to="/register" className="btn btn-light">
							<i className="fas fa-user-plus"></i> Sign up User
						</Link>
						{/* <li><Link to="/register">Sign up</Link></li> */}
						<Link to="/admin-add-package" className="btn btn-light">
							<i className="fa fa-plus"></i> Add New Package
						</Link>
						<Link to="/admin-update-session" className="btn btn-light">
							<i className="fa fa-plus"></i> Update sessions
						</Link>
					</div>
					<input
					...
					<div className="package-container">
						{userData.length > 0 ? (
							userData.map((client) => {
								return <PackagesItem key={client._id} client={client} />;
							})
						) : (
