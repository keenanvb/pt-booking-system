...
const CalendarManageBooking = ({
	adminBookingManage: { bookingEvents, loading },
	...
	const formateBookingEvents = (bookingEvents) => {
		...
		bookingEvents.forEach((bookingEvent, index) => {
			...
			newBookingEvents[index] = {
				...newBookingEvents[index],
				title: bookingEvent.confirmation
			...
			<form className="form" onSubmit={(e) => e.preventDefault()}>
								...
								{adminBookings.map((booking, index) => {
									return (
										<div className="list-item" key={index}>
											<h1 className="list-item-heading">
												{moment(booking.start).format('MMM Do YYYY, h:mm a')}
											</h1>
											<div
												className="btn-close"
												onClick={() => {
				...
				{adminBookings.length > 0 ? (
					<div style={{ position: 'relative' }}>
						<Link
							to={{
								pathname: `/manage-booking-client/`,
								state: {
...
export default connect(mapStateToProps, {
	...
	removeAdminBooking,
	setAlert,
})(withRouter(CalendarManageBooking));
